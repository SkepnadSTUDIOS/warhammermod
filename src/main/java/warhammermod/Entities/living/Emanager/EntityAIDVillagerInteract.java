...
public class EntityAIDVillagerInteract extends EntityAIWatchClosest2
{
    /** The delay before the villager throws an itemstack (in ticks) */
    private int interactionDelay;
    private final EntityDwarf villager;

    public EntityAIDVillagerInteract(EntityDwarf villagerIn)
    {
        super(villagerIn, EntityDwarf.class, 3.0F, 0.02F);
    ...
    public void startExecuting()
    {
        ...
        if (this.villager.canAbondonItems() && this.closestEntity instanceof EntityDwarf && ((EntityDwarf)this.closestEntity).wantsMoreFood())
        {
            ...
            if (this.interactionDelay == 0)
            {
                ...
                for (int i = 0; i < inventorybasic.getSizeInventory(); ++i)
                {
                    ...
                    if (!itemstack.isEmpty())
                    {
                        ...
                        if ((item == Items.BREAD ||item == Itemsinit.beer|| item == Items.POTATO || item == Items.CARROT || item == Items.BEETROOT) && itemstack.getCount() > 3)
                        {
                        ...
                        else if (item == Items.WHEAT && itemstack.getCount() > 5)
                        {
                            ...
                            int k = j / 3;
                            itemstack.shrink(j);
                            itemstack1 = new ItemStack(Itemsinit.beer, k, 0);
                        }
